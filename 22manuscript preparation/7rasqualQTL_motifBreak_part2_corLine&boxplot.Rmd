...
out.dir <- '~/yuzhao1/work/atac_gca2024/0manu/plots/7rasqualQTL_motifBreak_part2/linear_cor_example/'
```
...
correlation_dir <- '~/yuzhao1/work/atac_gca2024/0manu/plots/7rasqualQTL_motifBreak_part1/'
...
```{r}
for (ct in celltypes) {
  rasqual_lead <- readRDS(paste0(rasqual_dir, ct, '_lead.rds')) %>% data.frame()
  rasqual_lead$qtl_pair <- paste0(rasqual_lead$Feature_ID, '@', rasqual_lead$rsID)
  rownames(rasqual_lead) <- rasqual_lead$qtl_pair
  ...
  for (tf in rownames(df_adjp)) {
    ...
    p <- ggplot(df, aes(x=disruption_score_alleleDiff, y=Effect_size)) + 
      ...
      geom_point(data=df[df$rsID=='chr12:52454608:T:C', ], aes(x=disruption_score_alleleDiff, y=Effect_size), colour="#b2182b", size=1) + # select the point to highlight
    ...
    ### section 2: plot top QTL snps, select top/bottom n snps to plot
    df_fix1 <- data.frame(df)
    nTopQTL <- min(1, nrow(df_fix1))
    df_plot <- df_fix1[order(df_fix1$Effect_size, decreasing = F), ]
    ...
    for(i in 1:nrow(df_plot)){
        ...
        # plot q
        rasq.q <- rasqual_lead[qtl_pair_name, 'q'] %>% format(.,  digits = 2, scientific=T)
        ...
        p <- ggplot(df, aes(x=genotype, y=peak_counts, fill=genotype))+
            ...
            theme(axis.text.x = element_text(color = "black", size = 6),
                ...
                axis.title = element_text(color = "black", size = 6, angle = 0),
            ...
            labs(x = qtl_pair_name, y = "Normalized accessibility", title = paste0(rsID_standard, ', q=', rasq.q))
