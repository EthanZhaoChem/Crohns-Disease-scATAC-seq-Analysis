...
get_gene_annotations <- function(
  ...
  single.strand.genes.only = T
){
  
  cat("Get genes from TxDb...\n")
  if(single.strand.genes.only == F){
    genes        <- suppressMessages(GenomicFeatures::genes(TxDb, single.strand.genes.only=F))
    geneIDs_all <- genes@elementMetadata$gene_id
    geneID_map   <- map_geneIDs(OrgDb, geneIDs_all, keytype_geneID, columns_extract)
    return(geneID_map)
  }
  
  if(single.strand.genes.only == T){
    genes        <- suppressMessages(GenomicFeatures::genes(TxDb, single.strand.genes.only=T))
