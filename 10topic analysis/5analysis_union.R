...
# fixed
lineage <- 'union'
lineage.anno1 <- gca_colors_atac_union_anno1
...
load(paste0(data.dir, "union_raw_sub100.RData"))
...
# fixed
file_fasttopic <- paste0('~/yuzhao1/work/atac_gca2024/13fasttopic/rds/fit_union_sub100_k',  nTopics, '_converged.rds')
file_fasttopic_de <- paste0('~/yuzhao1/work/atac_gca2024/13fasttopic/rds/fit_union_sub100_k',  nTopics, '_converged_de_default.rds')
plot.dir <- paste0('~/yuzhao1/work/atac_gca2024/13fasttopic/plots/union_sub100_k', nTopics, '/')
...
pdf(paste0(plot.dir, lineage, '_sub', subID, '_k_',  nTopics, '_topic_corr.pdf'), height = 13, width = 17)
ht <- Heatmap(corr_mtx,
...
# only need to customize the topics you want to show (make sure it's sorted)
# work on colors, last one is white
# plot presets
plot.titles_list <- list('inflammatory', 'epithelial', 'adaptive_immune', 
                         'innate_immune', 'stromal',
                         'customized')
partial_topics_list <- list(c(7, 9, 17, 21, 40),
                       c(5, 6, 8, 23, 24, 26, 27, 29, 30, 37, 43),
                       c(10, 13, 14, 44, 36, 19, 45, 32, 35),
                       c(1, 15, 18, 25),
                       c(12, 28, 39, 41),
                       c(5, 13, 18, 23, 29, 44))
                       
colors_pool_list <- list(c("gold", '#33a02c', "#ff7f00", '#df65b0', '#1f78b4'),
                    c('#8dd3c7','#f781bf','#bebada','#fb8072','#80b1d3', '#fccde5', '#fdb462','#b3de69', '#1a9850','#35978f', '#1f78b4'),
                    c('#80b1d3', '#33a02c','#fdb462','#b3de69', '#8dd3c7','#35978f', '#1f78b4', '#f4a582', '#9970ab'),
...
# start
i <- 6
plot.title <- plot.titles_list[[i]]
partial_topics <- partial_topics_list[[i]]
colors_pool <- colors_pool_list[[i]]

topic_colors <- rep('white', 1+length(partial_topics))
topic_colors[1:(length(topic_colors)-1)] <- colors_pool[1:(length(topic_colors)-1)]

# work on scores, last column is merged white, the columns should be in correct order as specified
labels <- factor(metadata1$anno1, levels = names(lineage.anno1))
...
colnames(fit2$L)[length(partial_topics) + 1] <- "other"
fit2$L <- fit2$L[, c(paste0("k", partial_topics), 'other')] # this made sure the updated score are in the correct order above

# # if only plot a subset of cells
# labels_order <- c()
# for (ct in names(lineage.anno1)) {
#   labels_order <- c(labels_order, paste0(ct, '-', c( "Control", "nonInf",  "adjInf",  "inf")))
# }
# labels_order <- c()
# for (tmp in c("Control", "nonInf",  "adjInf",  "inf")) {
#   labels_order <- c(labels_order, paste0(names(lineage.anno1), '-', tmp))
# }
# labels <- factor(paste0(metadata1$anno1, '-', metadata1$inflammation_status), levels = labels_order)


# plot
set.seed(2)
pdf(paste0(plot.dir, 'structure_plot/', plot.title, '.pdf'), height = 3, width = 10)
...
png(paste0(plot.dir, 'da_plot_sub', subID, '_k_', nTopics, '_lpval.png'), height = 5000, width = 7200, res = 300)
do.call(plot_grid, c(plots, nrow = 6, ncol = 5))
dev.off()
...
for (k in 1:nTopics) {
  ...
  plots[[k]] <- ggplot(data, aes(x = lpval)) +
    geom_histogram(bins = 1000, fill = "blue", color = "black") +
    geom_vline(xintercept = -log10(0.05), color = "#b2182b", linetype = "dashed", size = 0.5) +
    geom_vline(xintercept = x_vertical, color = "#1a9850", linetype = "dashed", size = 0.5) +
    xlim(0, 60) +
    ggtitle(paste("Topic", k))
}
png(paste0(plot.dir, 'da_vsnull_plot_sub', subID, '_k_', nTopics, '_lpval_top5k.png'), height = 4000, width = 7200, res = 300)
...
daPeaks.dir <- paste0(plot.dir, 'daPeaks_positive_flexibleLpval100_vsnull/')
dir.create(daPeaks.dir)
# save da peak to bed files
for (i in 1:nTopics){
  peaks <- da_peaks[[i]]
...
library(openxlsx)
names(motifs_perTopic_daPeaks) <- paste0('Topic', 1:nTopics, '_', lengths(da_peaks), 'peaks')
write.xlsx(motifs_perTopic_daPeaks, file = paste0(plot.dir, 'enriched_motif_from_daPeaks_vsnull_positive_', 'sub', subID, '_k_', nTopics, '_top100.xlsx'))



################### topic score umap distribution  ##########################
umap_df <- proj@embeddings$LSI_UMAP$df
df <- bind_cols(cell_topic_mtx, metadata1[rownames(cell_topic_mtx),], umap_df[rownames(cell_topic_mtx), ])
...
df$embedding1 <- df$`IterativeLSI#UMAP_Dimension_1`
df$embedding2 <- df$`IterativeLSI#UMAP_Dimension_2`
df$cluster_name <- df$anno1

for (i in 1:nTopics) {
  df$feature_to_plot <- df[, paste0('k', i)]
  p <- plot_df_umap_custom(df, plot_feature = T, cap1 = 0.001, cap2 = 0.999)
  pdf(paste0(plot.dir, 'umap/topic', i, '.pdf'), height = 7, width = 6)
  print(p)
  dev.off()
}
...
for (i in 1:nTopics) {
  ...
  pdf(paste0(plot.dir, 'umap_inf/topic', i, '.pdf'), height = 13, width = 12)
  print(p)
  dev.off()
}
...
for (i in 1:nTopics) {
  ...
  pdf(paste0(plot.dir, 'umap_loc/topic', i, '.pdf'), height = 7, width = 10)
  print(p)
  dev.off()
}
